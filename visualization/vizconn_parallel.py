...
#     def _get_edge_path(self, i: int, j: int, use_arcs: bool=True) -> np.ndarray:
#         p0 = self.locs[i]
#         p1 = self.locs[j]
#         if use_arcs:
...
#     def _compute_edge_for_parallel(self, 
#         i, j, C, labels, directed, arc_radius, n_pts,
#         scale, zmin, zmax, colorscale,
#         edge_width_range, edge_opacity,
#         default_pos_color, default_neg_color,
#     ):
#         """Compute path, width, color, and arrow info for one edge."""
#         w = C[i, j]

#         # --- Compute color ---
#         color = helpers._get_edge_color(
#             edge_weight=w, zmin=zmin, zmax=zmax,
#             colorscale=colorscale,
#             default_pos_color=default_pos_color,
#             default_neg_color=default_neg_color,
#         )
...
#         # --- Path ---
#         P = self._get_edge_path(i, j, use_arcs=True)

#         # --- Arrow ---
#         arrow = None
#         if directed and len(P) >= 2:
#             q0, q1 = P[-2], P[-1]
#             arrow = (q0, q1, color, width)

#         return (i, j, P, w, color, width, arrow)
...
#         # --- Get ij pairs ---
#         ij_list = list(helpers._get_ij_iter(n_nodes, directed))

#         # --- Parallel compute chunks ---
#         results = Parallel(n_jobs=-1, prefer="threads")(
#             delayed(self._compute_edge_for_parallel)(
#                 i, j, C, labels, directed,
#                 self.n_arc_points,
#                 scale, zmin, zmax, colorscale,
#                 self.edge_width_range, self.edge_opacity,
#                 self.default_pos_color, self.default_neg_color,
#             )
#             for (i, j) in ij_list
#         )

#         # --- Add traces in main thread ---
#         for (i, j, P, w, color, width, arrow) in results:
...
#     def _compute_edge_update(i, j, idx, C, new_mask, directed,
#                             zmin, zmax, colorscale, scale,
#                             edge_width_range, default_neg_color, default_pos_color,
#                             get_path_fn):

#         w = C[i, j]
#         m = new_mask[i, j]  # visibility mask

#         # If edge not visible → return None (to hide)
#         if not m:
#             return idx, None
...
#             # ---------------------------------------------------------
#             #     DETERMINE WHICH EDGES NEED UPDATING (Plotly idxs)
#             # ---------------------------------------------------------
...
#             # ---------------------------------------------------------
#             #     PARALLEL PHASE: Compute path, width, color, weight
#             # ---------------------------------------------------------
#             results = Parallel(n_jobs=-1, backend="loky")(
#                 delayed(_compute_edge_update)(
#                     i, j, idx,
#                     C,
#                     new_thresh_mask,
#                     directed,
...
#                     self.default_neg_color,
#                     self.default_pos_color,
#                     self._get_edge_path,
#                 )
...
#             # ---------------------------------------------------------
#             #     MAIN THREAD: Apply results to Plotly traces safely
#             # ---------------------------------------------------------
#             for ((i, j), idx), (_, res) in zip(traces_list, results):
...
#                             fig.layout.annotations[ann_idx].visible = False
...
#             # ---------------------------------------------------------
#             helpers._update_colorbar(
#                 fig,
#                 self._colorbar_trace_idx,
#                 colorscale,
#                 zmin,
#                 zmax
#             )
...
#         and bulges outward from the origin.
#         """
#         p0 = self.locs[i]
#         p1 = self.locs[j]
...
#         # ---- 1) Construct the plane defined by (origin, p0, p1) ----
#         plane_normal = np.cross(p0, p1)
#         n_norm = np.linalg.norm(plane_normal)

#         # If p0 and p1 are colinear with origin — fallback
#         if n_norm < 1e-12:
#             plane_normal = np.cross(chord, np.array([0., 0., 1.]))
#             n_norm = np.linalg.norm(plane_normal)
#             if n_norm < 1e-12:
#                 plane_normal = np.cross(chord, np.array([1., 0., 0.]))
#                 n_norm = np.linalg.norm(plane_normal)

#         plane_normal /= n_norm

#         # ---- 2) Direction from p0→p1 along the plane (unit) ----
#         d = chord / L

#         # ---- 3) Direction for arc height = in plane & ⟂ chord ----
#         # This is the "outward bulge" direction.
#         perp = np.cross(plane_normal, d)
#         perp /= np.linalg.norm(perp)

#         # Make sure perp points AWAY from origin
#         if np.dot(perp, (p0 + p1) * 0.5) < 0:
#             perp = -perp

#         # ---- 4) Arc height (user controlled or automatic) ----
